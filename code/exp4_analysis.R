...
# load data
subs = list.files(path = "WM_adapt/data/exp4/", include.dirs = TRUE, pattern = '[0-9]{6}[A-Za-z]{2}$')
...
for (s in seq_along(subs)) {
  subdf = read.csv(file = file.path('WM_adapt/data/exp4',subs[s],paste0(subs[s],'.csv')),na.strings = 'nan')
...
if (!file.exists('WM_adapt/data/exp4/exp4_group_fixed.csv')) {
  write.csv(grpdf,
            file = 'WM_adapt/data/exp4/exp4_group_fixed.csv',
            row.names = F)
}
if (!file.exists('WM_adapt/data/exp4/exp4_group_random.csv')) {
  write.csv(grprandf,
            file = 'WM_adapt/data/exp4/exp4_group_random.csv',
            row.names = F)
}
...
if (!file.exists('WM_adapt/data/exp4/exp4_group_mean_fixed.csv')) {
  write.csv(mndf,
            file = 'WM_adapt/data/exp4/exp4_group_mean_fixed.csv',
            row.names = F)
}

# linear fit
if (!file.exists('WM_adapt/model_fits/Exp4_linear_fit_group_mean.rds')) {
  ...
  saveRDS(fit_linear,file = 'WM_adapt/model_fits/Exp4_linear_fit_group_mean.rds')
} else {
  fit_linear = readRDS('WM_adapt/model_fits/Exp4_linear_fit_group_mean.rds')
}

#  single exponential decay fit (Robinson, Soetedjo & Noto, 2006)
if (!file.exists('WM_adapt/model_fits/Exp4_expdecay_fit_group_mean.rds')) {
  ...
  fit_single = brm(bf(memResponse ~ (amp * (2^(-trial_adapt/rate))) + asymptote, amp + rate + asymptote ~ 1, nl = TRUE),
                   ...
                   chains = 4,
                   control = list(adapt_delta = 0.8))
  saveRDS(fit_single,file = 'WM_adapt/model_fits/Exp4_expdecay_fit_group_mean.rds')
} else {
  fit_single = readRDS('WM_adapt/model_fits/Exp4_expdecay_fit_group_mean.rds')
}

# double exponential
if (!file.exists('WM_adapt/model_fits/Exp4_dblexpdecay_fit_group_mean.rds')) {
  ...
  fit_dbl = brm(bf(memResponse ~ (amp1 * (2^(-trial_adapt/rate1))) + (amp2*(2^(-trial_adapt/rate2))) + plateau , amp1 + amp2 + rate1 + rate2 + plateau ~ 1, nl = TRUE),
                ...
                chains = 4,
                control = list(adapt_delta = 0.8))
  saveRDS(fit_dbl,file = 'WM_adapt/model_fits/Exp4_dblexpdecay_fit_group_mean.rds')
} else {
  fit_dbl = readRDS('WM_adapt/model_fits/Exp4_dblexpdecay_fit_group_mean.rds')
}

# model comparison
exp4_loo = loo(fit_linear,fit_single,fit_dbl)
exp4_loo

# WM-random bin analysis (adaptation field)
ranbindf = grprandf %>% mutate(bin = cut(memTargetPos,breaks = c(-Inf,seq(4.5,10.5,length.out = 7),Inf))) %>%
...
levels(blockdiffdf$bin_index) = c('-5','-4','-3','-2','-1','0','+1','+2')
