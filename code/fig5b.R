...
if (!file.exists('WM_adapt/data/exp4/exp4_group_random.csv')) {
  source('WM_adapt/code/exp4_analysis.R') # group aggregated WM-random data frame saved by this script
}
grprandf = read.csv('WM_adapt/data/exp4/exp4_group_random.csv')

# bin WM-random location
ranbindf = grprandf %>% 
  mutate(bin = cut(memTargetPos,breaks = c(-Inf,seq(4.5,10.5,length.out = 7),Inf))) %>%
...
blockdiffdf = ranbindf %>% 
  ...
  mutate(ydiff = `4` - `1`)
levels(blockdiffdf$bin) =  c('-5','-4','-3','-2','-1','0','+1','+2')
