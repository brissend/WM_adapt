...
if (!file.exists('WM_adapt/data/exp3/exp3_group_fixed.csv')) {
  source('WM_adapt/code/exp3_analysis.R') # subject recall computed and saved by this script
}
grpdf = read.csv('WM_adapt/data/exp3/exp3_group_fixed.csv')
...
for (ii in 1:length(unique(grpdf$subject))) {
  ...
  # linear model best fit group data
  set.seed(1111)
  fit_adapt[[ii]] = brm(y ~ trial,
  ...
  fit_postadapt[[ii]] = brm(y ~ trial_post,
  ...
  for (x in seq(minval,maxval,length.out = 20)) {
    ...
    tmp = epred_draws(fit_preadapt[[ii]],newdata = data.frame(x = x), ndraws = 5000)
  ...
  for (x in seq(minval,maxval,length.out = 100)) {
    ...
    tmp = epred_draws(fit_adapt[[ii]],newdata = data.frame(trial = x), ndraws = 5000)
  ...
  for (x in seq(minval,maxval,length.out = 20)) {
    ...
    tmp = epred_draws(fit_postadapt[[ii]],newdata = data.frame(trial_post = x), ndraws = 5000)
...
# save
saveRDS(fit_preadapt,file = 'WM_adapt/model_fits/Exp3_preadapt_linear_fit_subject.rds')
saveRDS(fit_adapt,file = 'WM_adapt/model_fits/Exp3_linear_fit_subject.rds')
saveRDS(fit_postadapt,file = 'WM_adapt/model_fits/Exp3_postadapt_linear_fit_subject.rds')
