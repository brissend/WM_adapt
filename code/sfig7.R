...
# ============================================
# Fit models (stimulus presentation + 100 ms) (Exp 4)
# ============================================
# linear fit
...
# ============================================
# Fit models (stimulus presentation + delay period) (Exp 4)
# ============================================
# linear fit
...
if (!file.exists('WM_adapt/data/exp5/exp5_nosaccade_stim_group_mean_fixed.csv')) {
  source('WM_adapt/code/exp5_eyetrack_analysis.R')
}
mndf_nosaccade_stim = read_csv('WM_adapt/data/exp5/exp5_nosaccade_stim_group_mean_fixed.csv')
mndf_nosaccade_delay = read_csv('WM_adapt/data/exp5/exp5_nosaccade_delay_group_mean_fixed.csv')

# ============================================
# Fit models (stimulus presentation + 100 ms) (Exp 5)
# ============================================
# linear fit
if (!file.exists('WM_adapt/model_fits/Exp5_nosaccade_stim_linear_fit_group_mean.rds')) {
  fit_linear = brm(memResponse ~ trial_adapt,
                   data = filter(mndf_nosaccade_stim,adaptfit == 1),
                   prior = set_prior("student_t(1,0,0.2)",class = "b"),
                   iter = 6000,
                   ...
                   chains = 4,
                   control = list(adapt_delta = 0.8))
  saveRDS(fit_linear,file = 'WM_adapt/model_fits/Exp5_nosaccade_stim_linear_fit_group_mean.rds')
} else {
  fit_linear = readRDS('WM_adapt/model_fits/Exp5_nosaccade_stim_linear_fit_group_mean.rds')
}

#  single exponential decay fit (Robinson, Soetedjo & Noto, 2006)
if (!file.exists('WM_adapt/model_fits/Exp5_nosaccade_stim_expdecay_fit_group_mean.rds')) {
  singleexpprior = prior(normal(0,5), nlpar = 'amp') +
    prior(normal(0,1000),nlpar = "rate",lb=0) +
    prior(normal(-9,5),nlpar = "asymptote")
  ...
  fit_single = brm(bf(memResponse ~ (amp * (2^(-trial_adapt/rate))) + asymptote, amp + rate + asymptote ~ 1, nl = TRUE),
                   ...
                   chains = 4,
                   control = list(adapt_delta = 0.8))
  saveRDS(fit_single,file = 'WM_adapt/model_fits/Exp5_nosaccade_stim_expdecay_fit_group_mean.rds')
} else {
  fit_single = readRDS('WM_adapt/model_fits/Exp5_nosaccade_stim_expdecay_fit_group_mean.rds')
}

# double exponential
if (!file.exists('WM_adapt/model_fits/Exp5_nosaccade_stim_dblexpdecay_fit_group_mean.rds')) {
  dbl_exp_prior = prior(normal(0,5), nlpar = 'amp1') +
    prior(normal(0,5), nlpar = 'amp2') + 
    prior(normal(0,50),nlpar = "rate1",lb = 0) +
    prior(normal(0,1000),nlpar = "rate2",lb = 0) + 
  ...
  fit_dbl = brm(bf(memResponse ~ (amp1 * (2^(-trial_adapt/rate1))) + (amp2*(2^(-trial_adapt/rate2))) + plateau , amp1 + amp2 + rate1 + rate2 + plateau ~ 1, nl = TRUE),
                ...
                chains = 4,
                control = list(adapt_delta = 0.8))
  saveRDS(fit_dbl,file = 'WM_adapt/model_fits/Exp5_nosaccade_stim_dblexpdecay_fit_group_mean.rds')
} else {
  fit_dbl = readRDS('WM_adapt/model_fits/Exp5_nosaccade_stim_dblexpdecay_fit_group_mean.rds')
}

# model comparison
exp5_nosaccade_stim_loo = loo(fit_linear,fit_single,fit_dbl)
exp5_nosaccade_stim_loo
fit_adapt = fit_dbl # double exponential fits best


# fit linear model to pre-adapt and post-adapt trials
if (!file.exists('WM_adapt/model_fits/Exp5_nosaccade_stim_preadapt_linear_fit_group_mean.rds')) {
  ...
  fit_preadapt = brm(memResponse ~ trial,
                     ...
                     chains = 4,
                     control = list(adapt_delta = 0.8))
  saveRDS(fit_preadapt,file = 'WM_adapt/model_fits/Exp5_nosaccade_stim_preadapt_linear_fit_group_mean.rds')
  
} else {
  fit_preadapt = readRDS('WM_adapt/model_fits/Exp5_nosaccade_stim_preadapt_linear_fit_group_mean.rds')
}

mndf_nosaccade_stim$trial_post = mndf_nosaccade_stim$trial - 880 
if (!file.exists('WM_adapt/model_fits/Exp5_nosaccade_stim_postadapt_linear_fit_group_mean.rds')) {
  ...
  fit_postadapt = brm(memResponse ~ trial_post,
                      ...
                      chains = 4,
                      control = list(adapt_delta = 0.8))
  saveRDS(fit_postadapt,file = 'WM_adapt/model_fits/Exp5_nosaccade_stim_postadapt_linear_fit_group_mean.rds')
  
} else {
  fit_postadapt = readRDS('WM_adapt/model_fits/Exp5_nosaccade_stim_postadapt_linear_fit_group_mean.rds')
}
...
ggplot(mndf_nosaccade_stim,aes(x = trial, y = memResponse,group = phase )) + 
  ...
  scale_shape_discrete(name = 'Phase') + 
  scale_x_continuous(breaks= seq(0,1000,by=250)) + 
  scale_y_continuous(breaks =  seq(-9.6,-7.2,by = 0.6),labels = paste0(seq(20,-60,by = -20),'%'),limits = c(-9.9, -6.9)) +  
  labs(color = 'Block',y = 'Recall Location (% of Backstep)', x = 'Trial') +
  theme_classic() +
  theme(legend.position = 'none')

# ============================================
# Fit models (stimulus presentation + delay period) (Exp 5)
# ============================================
# linear fit
if (!file.exists('WM_adapt/model_fits/Exp5_nosaccade_delay_linear_fit_group_mean.rds')) {
  fit_linear = brm(memResponse ~ trial_adapt,
                   data = filter(mndf_nosaccade_delay,adaptfit == 1),
                   prior = set_prior("student_t(1,0,0.2)",class = "b"),
                   iter = 6000,
                   ...
                   chains = 4,
                   control = list(adapt_delta = 0.8))
  saveRDS(fit_linear,file = 'WM_adapt/model_fits/Exp5_nosaccade_delay_linear_fit_group_mean.rds')
} else {
  fit_linear = readRDS('WM_adapt/model_fits/Exp5_nosaccade_delay_linear_fit_group_mean.rds')
}

#  single exponential decay fit (Robinson, Soetedjo & Noto, 2006)
if (!file.exists('WM_adapt/model_fits/Exp5_nosaccade_delay_expdecay_fit_group_mean.rds')) {
  singleexpprior = prior(normal(0,5), nlpar = 'amp') +
    prior(normal(0,1000),nlpar = "rate",lb=0) +
    prior(normal(9,5),nlpar = "asymptote")
  ...
  fit_single = brm(bf(memResponse ~ (amp * (2^(-trial_adapt/rate))) + asymptote, amp + rate + asymptote ~ 1, nl = TRUE),
                   ...
                   chains = 4,
                   control = list(adapt_delta = 0.8))
  saveRDS(fit_single,file = 'WM_adapt/model_fits/Exp5_nosaccade_delay_expdecay_fit_group_mean.rds')
} else {
  fit_single = readRDS('WM_adapt/model_fits/Exp5_nosaccade_delay_expdecay_fit_group_mean.rds')
}

# double exponential
if (!file.exists('WM_adapt/model_fits/Exp5_nosaccade_delay_dblexpdecay_fit_group_mean.rds')) {
  dbl_exp_prior = prior(normal(0,5), nlpar = 'amp1') +
    prior(normal(0,5), nlpar = 'amp2') + 
    prior(normal(0,50),nlpar = "rate1",lb = 0) +
    prior(normal(0,1000),nlpar = "rate2",lb = 0) + 
  ...
  fit_dbl = brm(bf(memResponse ~ (amp1 * (2^(-trial_adapt/rate1))) + (amp2*(2^(-trial_adapt/rate2))) + plateau , amp1 + amp2 + rate1 + rate2 + plateau ~ 1, nl = TRUE),
                ...
                chains = 4,
                control = list(adapt_delta = 0.8))
  saveRDS(fit_dbl,file = 'WM_adapt/model_fits/Exp5_nosaccade_delay_dblexpdecay_fit_group_mean.rds')
} else {
  fit_dbl = readRDS('WM_adapt/model_fits/Exp5_nosaccade_delay_dblexpdecay_fit_group_mean.rds')
}

# model comparison
exp5_nosaccade_delay_loo = loo(fit_linear,fit_single,fit_dbl)
exp5_nosaccade_delay_loo
fit_adapt = fit_single # exponential models indistinguishable (using simpler single exponential model)


# fit linear model to pre-adapt and post-adapt trials
if (!file.exists('WM_adapt/model_fits/Exp5_nosaccade_delay_preadapt_linear_fit_group_mean.rds')) {
  ...
  fit_preadapt = brm(memResponse ~ trial,
                     ...
                     chains = 4,
                     control = list(adapt_delta = 0.8))
  saveRDS(fit_preadapt,file = 'WM_adapt/model_fits/Exp5_nosaccade_delay_preadapt_linear_fit_group_mean.rds')
  
} else {
  fit_preadapt = readRDS('WM_adapt/model_fits/Exp5_nosaccade_delay_preadapt_linear_fit_group_mean.rds')
}

mndf_nosaccade_delay$trial_post = mndf_nosaccade_delay$trial - 880 
if (!file.exists('WM_adapt/model_fits/Exp5_nosaccade_delay_postadapt_linear_fit_group_mean.rds')) {
  ...
  fit_postadapt = brm(memResponse ~ trial_post,
                      ...
                      chains = 4,
                      control = list(adapt_delta = 0.8))
  saveRDS(fit_postadapt,file = 'WM_adapt/model_fits/Exp5_nosaccade_delay_postadapt_linear_fit_group_mean.rds')
  
} else {
  fit_postadapt = readRDS('WM_adapt/model_fits/Exp5_nosaccade_delay_postadapt_linear_fit_group_mean.rds')
}
