...
if (!file.exists('WM_adapt/data/exp3/exp3_group_mean_fixed.csv')) {
  source('WM_adapt/code/exp3_analysis.R') # group average recall computed and saved by this script
}
mndf = read.csv('WM_adapt/data/exp3/exp3_group_mean_fixed.csv')

# load model fit
fit_adapt = readRDS('WM_adapt/model_fits/Exp3_linear_fit_group_mean.rds')

# fit linear model to pre-adapt and post-adapt trials
if (!file.exists('WM_adapt/model_fits/Exp3_preadapt_linear_fit_group_mean.rds')) {
  ...
  saveRDS(fit_preadapt,file = 'WM_adapt/model_fits/Exp3_preadapt_linear_fit_group_mean.rds')
  
} else {
  fit_preadapt = readRDS('WM_adapt/model_fits/Exp3_preadapt_linear_fit_group_mean.rds')
}

mndf$trial_post = mndf$x - 880 
if (!file.exists('WM_adapt/model_fits/Exp3_postadapt_linear_fit_group_mean.rds')) {
  ...
  fit_postadapt = brm(y ~ trial_post,
                      ...
                      chains = 4,
                      control = list(adapt_delta = 0.8))
  saveRDS(fit_postadapt,file = 'WM_adapt/model_fits/Exp3_postadapt_linear_fit_group_mean.rds')
  
} else {
  fit_postadapt = readRDS('WM_adapt/model_fits/Exp3_postadapt_linear_fit_group_mean.rds')
}
