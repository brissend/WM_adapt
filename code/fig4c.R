...
# load model fit
fit_adapt = readRDS('WM_adapt/model_fits/exp5_dblexpdecay_fit_group_mean.rds')

# fit linear model to pre-adapt and post-adapt trials
if (!file.exists('WM_adapt/model_fits/exp5_preadapt_linear_fit_group_mean.rds')) {
  ...
  saveRDS(fit_preadapt,file = 'WM_adapt/model_fits/exp5_preadapt_linear_fit_group_mean.rds')
  
} else {
  fit_preadapt = readRDS('WM_adapt/model_fits/exp5_preadapt_linear_fit_group_mean.rds')
}
...
if (!file.exists('WM_adapt/model_fits/exp5_postadapt_linear_fit_group_mean.rds')) {
  ...
  fit_postadapt = brm(y ~ trial_post,
                      ...
                      chains = 4,
                      control = list(adapt_delta = 0.8))
  saveRDS(fit_postadapt,file = 'WM_adapt/model_fits/exp5_postadapt_linear_fit_group_mean.rds')
  
} else {
  fit_postadapt = readRDS('WM_adapt/model_fits/exp5_postadapt_linear_fit_group_mean.rds')
}
