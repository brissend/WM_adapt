...
for (s in seq_along(subs)) {
  ...
  # fix scale of fixed response
  # slider physically scaled from 0.17 - 0.75 but psychopy allows you to specify the range independently of physical size
  # range was accidentally set to 0.17 - 0.7 so we need to rescale values to 0.17 to 0.75
  # this only affects the scaling of the data 
  # we can recover the actual screen coordinates of the participant's response using the below formula
  # formula: ((val - origmin)/(origmax - origmin)) * (scalemax - scalemin) + scalemin
  subfixeddf$y = ((subfixeddf$y - 0.17) / (0.7 - 0.17)) * (0.75 - 0.17) + 0.17
  
  # compute recall difference between successive WM-fixed trials
  subfixeddf$ydiff = c(0,diff(subdf$probe_slider_fixed.response[subdf$trialCounter %in% fixed_trial_nums]))
  ...
  print(subdf$quiz_slider_1.response[1109] < 3) # head movement question
  print(subdf$quiz_slider_2.response[1109] >= 8) # eye movement question
  print('')
...
if (!file.exists('WM_adapt/data/exp2/exp2_group_fixed.csv')) {
  write.csv(grpdf,
            file = 'WM_adapt/data/exp2/exp2_group_fixed.csv',
            row.names = F)
}
...
# Bayes factor examining linear effect of trial in pre-adapt phase
set.seed(1111)
grpdf$subject = factor(grpdf$subject)
bf_full = lmBF(y ~ x + subject,
               whichRandom = "subject",
               data = filter(grpdf,block == 1) %>% drop_na(y))
...
if (!file.exists('WM_adapt/model_fits/Exp2_linear_fit_group_mean.rds')) {
  ...
  saveRDS(fit_linear,file = 'WM_adapt/model_fits/Exp2_linear_fit_group_mean.rds')
} else {
  fit_linear = readRDS('WM_adapt/model_fits/Exp2_linear_fit_group_mean.rds')
}

#  single exponential decay fit (Robinson, Soetedjo & Noto, 2006)
if (!file.exists('WM_adapt/model_fits/Exp2_expdecay_fit_group_mean.rds')) {
  ...
  fit_single = brm(bf(y ~ (amp * (2^(-trial/rate))) + asymptote, amp + rate + asymptote ~ 1, nl = TRUE),
                   ...
                   chains = 4,
                   control = list(adapt_delta = 0.8))
  saveRDS(fit_single,file = 'WM_adapt/model_fits/Exp2_expdecay_fit_group_mean.rds')
} else {
  fit_single = readRDS('WM_adapt/model_fits/Exp2_expdecay_fit_group_mean.rds')
}

# double exponential
if (!file.exists('WM_adapt/model_fits/Exp2_dblexpdecay_fit_group_mean.rds')) {
  ...
  fit_dbl = brm(bf(y ~ (amp1 * (2^(-trial/rate1))) + (amp2*(2^(-trial/rate2))) + plateau , amp1 + amp2 + rate1 + rate2 + plateau ~ 1, nl = TRUE),
                ...
                chains = 4,
                control = list(adapt_delta = 0.8))
  saveRDS(fit_dbl,file = 'WM_adapt/model_fits/Exp2_dblexpdecay_fit_group_mean.rds')
} else {
  fit_dbl = readRDS('WM_adapt/model_fits/Exp2_dblexpdecay_fit_group_mean.rds')
}

# model comparison
exp2_loo = loo(fit_linear,fit_single,fit_dbl)
exp2_loo
